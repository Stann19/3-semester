...
template <typename T>
void sort(T* data, int array_size, Comparator<T>& comparator) {
    if (array_size <= 1)
        return;
    ...
    sort(data, ((char*)pivot - (void*)(char*)data) / element_byte_size, comparator);
    sort((T*)((char*)pivot + element_byte_size), array_size - 1 - ((char*)pivot - (void*)(char*)data) / element_byte_size, comparator);
}
...
int main()
{
    ...
    if (n == 0) {
        ...
        sort<point>(a, n_point, point_comparator);
        for (int i = 0; i < n_point; i++) {
    ...
    if (n == 1) {
        ...
        double_Comparator double_comparator;
        sort<double>(b, n_double, double_comparator);
        for (int i = 0; i < n_double; i++) {
            cout << b[i] << endl;
        }
        delete[] b;
